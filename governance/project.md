...
- `魂师修炼RPG_manifest.json` 仅作为兼容运行包的**派生导出视图**存在，由 `runtime/registry.json` 与当前源码生成；它不得反向成为 Git 源码权威。
...
- 生成视图只允许作为可再生消费者存在。兼容 manifest、兼容 runtime ZIP 都必须能从 Git 源码重新生成，并通过重开验证。
