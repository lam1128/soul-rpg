...
24. 每个CG收藏槽必须能回溯到唯一 `CHAPTER_ID` 的单章CG锚点；CG大厅/UI不得替换成同章其他场面、补造未发生的关键动作/物件/身体接触，也不得提供脱离篇章记录的独立解锁路径。
25. 若某章已具备CG解锁状态但单章缺少足够明确、可观察的对应CG锚点，增量QA应判为内容缺口并要求修复单章 owner；不得允许显示层临场创作一个替代画面来“补齐”。
...
- CG剧情绑定专项：每个已显示CG必须来自对应单章实际发生的CG锚点；显示层不得选取另一个“更漂亮”的剧情瞬间替代，也不得把未发生的动作、物件或亲密程度写进收藏描述。CG解锁只能是篇章进度/本章表现的派生结果。
