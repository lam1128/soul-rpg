...
def main():
    ...
    if current.get('dispatch_type')!='route' or current.get('route')!='readonly_query': fail('SAVE_CURRENT must be readonly_query route')
    if current.get('recognition_owner')!='13_交互协议.md': fail('SAVE_CURRENT recognition owner mismatch')
    if '§2C SAVE_CURRENT' not in current.get('recognition_sections',[]): fail('SAVE_CURRENT recognition section missing')
    if controls['SAVE_EXPORT'].get('priority',999) >= current.get('priority',999): fail('explicit SAVE_EXPORT must be checked before SAVE_CURRENT')
    readonly=r.get('topic_routes',{}).get('readonly_query',{})
    if readonly.get('state_access')!='READ_CONSUMER': fail('readonly_query state access changed')
    interaction=(p/'13_交互协议.md').read_text(encoding='utf-8')
    ui=(p/'UI_统一规范_日常恢复与菜单.md').read_text(encoding='utf-8')
    schema=(p/'12_状态存档.md').read_text(encoding='utf-8')
    if '## 2C. SAVE_CURRENT' in interaction: fail('unexpected obsolete SAVE_CURRENT heading format')
    if '## 2C. SAVE_CURRENT' not in interaction and '## 2C. SAVE_CURRENT｜Git 当前进度确认' not in interaction: fail('SAVE_CURRENT interaction contract missing')
    if '只有用户明确说“导出存档' not in interaction: fail('interaction does not separate save from export')
    if '## 16. 存档确认｜Git current state' not in ui: fail('Git save confirmation UI missing')
    if '单独的“存档 / 保存进度 / 存一下档”不得命中本事务' not in schema: fail('SAVE_EXPORT exclusion for bare save missing')
    if '不再承担“等到导出时才清理”的职责' not in schema: fail('canonical state cleanup still depends on export')
    if '不是 Git current state 的源码或运行权威' not in schema: fail('compatibility package metadata authority boundary missing')
    ...
    print(json.dumps({'status':'PASS','checkpoint_mode':'git_current_state','state_owner':owner,'state_rev':h['STATE_REV'],'save_current_route':current.get('route'),'save_skill':'soul-rpg-save-manager'},ensure_ascii=False))
